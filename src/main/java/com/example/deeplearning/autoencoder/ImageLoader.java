...
public class ImageLoader extends NativeImageLoader implements Serializable {
    /**
	 * 
	 */
    ...
    public ImageLoader(boolean train) {
        this( train, (File) null );
    }

    public ImageLoader(boolean train, File fullPath) {
        this( height, width, channels, train, fullPath, System.currentTimeMillis(), true );
    }

    public ImageLoader(int height, int width, int channels,  boolean train, File fullDir, long seed, boolean shuffle) {
    ...
    protected void load() {
        try {
            if (train) {
                ...
                Collection<File> trainFiles = FileUtils.listFiles( trainFile, new String[]{"jpg","png"}, true );
                fileIterator = trainFiles.iterator();
                numExample = trainFiles.size();
            } else {
                File testFile = new File(fullDir, "test");
                Collection<File> testFiles = FileUtils.listFiles(testFile, new String[]{"jpg","png"}, true );
